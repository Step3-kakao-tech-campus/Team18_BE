...
import com.example.demo.mentoring.dto.ContactResponse;
...
public class ContactService {
    ...
    public List<ContactResponse.ContactDashBoardMenteeDTO> findAllByMentee(User mentee) {
        ...
        return contactJPARepository.findAllByMenteeId(mentee.getId()).stream()
                .map(notConnectedRegisterUser -> createMenteeContactDTO(notConnectedRegisterUser.getMentoringBoard(), notConnectedRegisterUser))
                .collect(Collectors.toList());
    }

    // contact - mentee 부분 리팩토링 ( DTO 를 만드는 부분 )
    private ContactResponse.ContactDashBoardMenteeDTO createMenteeContactDTO(MentoringBoard mentoringBoard, NotConnectedRegisterUser contactUser) {
        ...
        return new ContactResponse.ContactDashBoardMenteeDTO(mentoringBoard, contactMentorDTO, contactUser);
    }
    ...
    // MentorPostDTO 생성 로직
    private ContactResponse.ContactDashboardMentorDTO createMentorPostDTO(MentoringBoard mentoringBoard, ContactResponse.ContactMentorDTO contactMentorDTO) {
        List<ContactResponse.ConnectionDTO> connectionDTOS = contactJPARepository.findAllByMentorPostId(mentoringBoard.getId())
                .stream()
                .map(this::createMenteeDTO)
                .collect(Collectors.toList());

        return new ContactResponse.ContactDashboardMentorDTO(mentoringBoard, contactMentorDTO, connectionDTOS);
    }
    ...
    public void acceptContact(ContactRequest.ContactAcceptDTO contactAcceptDTO, User mentor) {
        ...
        // 현재 멘토가 작성한 글인지 체크
        MentoringBoard mentoringBoard = mentorPostJPARepository.findById(mentorPostId)
        ...
        for ( ContactRequest.ContactAcceptDTO.AcceptMenteeDTO acceptMenteeDTO : contactAcceptDTO.getMentees() ) {
            ...
            doneJPARepository.save(new ConnectedUser(mentoringBoard, notConnectedRegisterUser.getMenteeUser()));
        }
    ...
    public void createContact(ContactRequest.ContactCreateDTO contactCreateDTO, User mentee) {
        ...
        contactJPARepository.save(new NotConnectedRegisterUser(mentoringBoard, mentee, ContactStateEnum.AWAIT));
